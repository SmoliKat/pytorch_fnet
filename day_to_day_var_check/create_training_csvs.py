...
def save_csvs(data_manifest, filename, image_save_dir, structure_display_name,save_path):
...
def create_slurm_runner(base_dir,structure_display_name_formated , exp_list,prefs_file_name='prefs.json'):
    with open(os.path.join(base_dir,structure_display_name_formated,"exp_runner.sh"), "w") as file_object:
        for exp in exp_list:
            running_line = f'sbatch train_mk3.sh "{os.path.join(base_dir,exp,prefs_file_name)}" "{exp}"'
...
def main():
    ...
    for structure_display_name in unique_structures:
        exp_list = []
        if not os.path.exists(os.path.join(args.base_exp_path, structure_display_name.replace(" ", "-"))):
            only_needed_data = whole_data_manifest.loc[whole_data_manifest['Structure'] == structure_display_name]
            # only_needed_data = whole_data_manifest.loc[whole_data_manifest['Structure'] == 'Microtubules']
            ...
            patterns = [{"train": [date_range[0], date_range[1], date_range[2]], "test": date_range[3]},
                        ...
                        {"train": [date_range[1], date_range[2], date_range[3]], "test": date_range[0]},
                        {"train": [date_range[1], date_range[2], date_range[3], date_range[0]], "test": date_range[0]},
                        {"train": [date_range[0], date_range[2], date_range[3]], "test": date_range[1]},
                        {"train": [date_range[0], date_range[2], date_range[3], date_range[1]], "test": date_range[1]},
                        {"train": [date_range[0], date_range[1], date_range[3]], "test": date_range[2]},
                        {"train": [date_range[0], date_range[2], date_range[3], date_range[1]], "test": date_range[2]},
                        ]
            ...
            for pattern in patterns:
                X_test = pd.DataFrame(columns=[col for col in only_needed_data.columns])
                X_train = pd.DataFrame(columns=[col for col in only_needed_data.columns])
                run_data= only_needed_data.copy(deep=True)
                date_to_test = pattern["test"]
                if len(run_data.loc[run_data.date == date_to_test]) > n_test_images:
                    for index in range(0, n_test_images):
                        item = run_data.loc[run_data.date == date_to_test].iloc[:1]
                        X_test = X_test.append(item)
                        run_data.drop(
                            run_data.index[run_data.index.tolist().index(item.index[0])], inplace=True)
                else:
                    raise (Exception(
                        f"not enough images only {len(run_data.loc[run_data.date == date_to_test])} for date {date_to_test}"))
                counter = 0
                while counter < n_train_images:
                    for date in pattern["train"]:
                        if counter < n_train_images:
                            item = run_data.loc[run_data.date == date].iloc[:1]
                            if item is not None:
                                X_train = X_train.append(item)
                                counter += 1
                                run_data.drop(
                                    run_data.index[run_data.index.tolist().index(item.index[0])],inplace=True)
                structure_display_name_formated = structure_display_name.replace(" ", "-")
                exp_name = get_exp_name(structure_display_name_formated, pattern)
                save_path = os.path.join(args.base_exp_path, structure_display_name_formated, exp_name)
                train_csv_path = save_csvs(data_manifest=X_train,save_path=save_path,
                          filename="image_list_train.csv", image_save_dir=args.image_save_dir,
                          structure_display_name=structure_display_name)
                test_csv_path = save_csvs(data_manifest=X_test, save_path=save_path,
                          filename="image_list_test.csv", image_save_dir=args.image_save_dir,
                          structure_display_name=structure_display_name)

                #pref file
                prefs_save_path = os.path.join(args.base_exp_path, structure_display_name_formated ,exp_name, 'prefs.json')
                if not (os.path.exists(prefs_save_path)):
                    ...
                    with open(prefs_save_path, "w") as fp:
                        json.dump(prefs, fp)
                exp_list.append(os.path.join(structure_display_name_formated,exp_name))
            create_slurm_runner(args.base_exp_path,structure_display_name_formated , exp_list)
